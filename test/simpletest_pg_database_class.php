...
class TestOfPgDatabaseClass extends UnitTestCase {
  ...
  function test_create() {
    $this->assertEqual(self::do_sql('DROP TABLE pg_test'), 'ERROR:  table "pg_test" does not exist');
    $this->assertEqual(self::do_sql('CREATE TABLE pg_test (tal integer, str varchar(40))'), 'ok');
  }

  function test_insert() {
    $this->pg->bind('tal', 1);
    $this->pg->bind('str', 'en');
    $this->assertEqual(self::do_sql("INSERT INTO pg_test VALUES (:tal, :str)"), 'ok');
    $this->assertEqual(self::do_sql("INSERT INTO pg_test VALUES (2, 'to')"), 'ok');
  }

  function test_select() {
    $this->assertEqual(self::do_sql('SELECT * FROM pg_test WHERE tal = 1'), 'ok');
  ...
  function test_select_with_bind() {
    $this->pg->bind('tal', 1);
    $this->pg->bind('str', 'en');
    $this->assertEqual(self::do_sql('SELECT tal FROM pg_test WHERE tal = :tal AND str = :str'), 'ok');
    $this->assertEqual($this->pg->num_rows(), 1);
  ...
  function test_update() {
    $this->assertEqual(self::do_sql("UPDATE pg_test SET str = 'toto' WHERE tal = 2"), 'ok');
    $this->assertEqual(self::do_sql('SELECT * FROM pg_test WHERE tal = 2'), 'ok');
    $this->assertEqual($this->pg->num_rows(), 1);
    $this->assertEqual($this->pg->get_row(), array('tal' => 2, 'str' => 'toto'));
  }

  function test_delete() {
    $this->assertEqual(self::do_sql('DELETE FROM pg_test WHERE tal = 1'), 'ok');
    $this->assertEqual(self::do_sql('SELECT * FROM pg_test'), 'ok');
    $this->assertEqual($this->pg->num_rows(), 1);
  }
  ...
  function test_transaction() {
    ...
    $this->assertEqual(self::do_sql("INSERT INTO pg_test VALUES (3, 'tre')"), 'ok');
    $this->assertEqual(self::do_sql("INSERT INTO pg_test VALUES (4, 'fire')"), 'ok');
  ...
  function test_cleanup() {
    $this->assertEqual(self::do_sql('DROP TABLE pg_test'), 'ok');
  }
